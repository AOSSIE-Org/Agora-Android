...
class CreateElectionTwo : Fragment() {
  ...
  override fun onCreateView(
    ...
    viewModel = ViewModelProviders.of(this, factor)
        .get(CreateElectionTwoViewModel::class.java)
    binding =
      DataBindingUtil.inflate(inflater, R.layout.create_election_two_fragment, container, false)
    ...
    binding.submitDetailsBtn.setOnClickListener({
      if (viewModel.getAllCandidates()!!.size != 0) {
        ...
        Navigation.findNavController(it)
            .navigate(R.id.action_election2_to_election3)
      } else {
        Toast.makeText(
